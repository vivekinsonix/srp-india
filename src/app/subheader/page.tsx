...
export default function HeroSection({
  backgroundImage,
  overlayGradient = "from-black/90 via-black/80 to-black/70",
  eyebrow,
  title,
  highlight,
  description,
  ...
  return (
    <section className="relative isolate overflow-hidden pt-5">
      ...
      <div className="mx-auto max-w-7xl px-4 py-20 md:py-28 text-white">
        ...
        {description && (
          <p className="mt-4 max-w-2xl text-white/90">{description}</p>
        )}
        <div className="mt-8 flex flex-wrap items-center gap-3">
          {primaryAction && (
            <a
              href={primaryAction.href}
              className="rounded-xl bg-teal-600 px-4 py-2 font-semibold hover:bg-teal-500"
            >
              {primaryAction.label}
            </a>
          )}
