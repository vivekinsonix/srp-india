...
const JOBS = [
  {
    ...
    description:
      "Own end-to-end call handling, load status updates, and CRM hygiene. Prior US logistics or brokerage BPO required.",
    responsibilities: [
      "Handle inbound/outbound calls via RingCentral with QA-ready notes",
      "Update load statuses; coordinate with US team on exceptions",
      "Maintain Zoho hygiene; log 100% interactions",
      "Coach juniors; uphold process SOPs",
    ],
    requirements: [
      "Excellent English (neutral accent)",
      "Hands-on with any CRM (Zoho preferred)",
      "TMS familiarity (Turvo preferred)",
      "Availability for US time zones",
    ],
  },
  {
    id: "qa-analyst",
    ...
    description:
      "Audit calls, score agents, and partner with ops for continuous improvement.",
    responsibilities: [
      "Calibrate QA scorecards and run audits",
      "Publish weekly quality insights and training needs",
      "Own coaching plans with Team Leads",
    ],
    requirements: [
      "Strong English and documentation",
      "Experience with RingCentral or similar",
      "BPO QA experience",
    ],
  },
];
...
const BLOGS = [
  {
    ...
    excerpt:
      "From a handful of trucks to a multi-business logistics brand—Mr. Boparai’s journey and social impact in Punjab.",
    content:
...
export default function SRPIndiaSite() {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return JOBS.filter(
      (j) => j.title.toLowerCase().includes(q) || j.tags.join(" ").toLowerCase().includes(q)
    );
  }, [jobQuery]);
...
<Header />
        ...
        <div className="mx-auto max-w-7xl px-4 py-20 grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="mt-10 grid grid-cols-3 gap-6">
              ...
              <Stat kpi="150k+" label="Shipments handled" />
            </div>
          </div>
          <div className="rounded-2xl bg-gradient-to-br from-slate-800 to-slate-700 p-6 shadow-xl min-h-[280px] flex items-center justify-center">
            <div className="text-center">
              <p className="text-sm uppercase tracking-widest text-teal-300">India Ops</p>
              <h3 className="mt-2 text-xl font-semibold">Freight Brokerage • US Accounting • IT/Admin • HR/Payroll</h3>
      ...
      <Section id="careers" title="Careers – Experienced US Logistics BPO">
        <div className="flex items-center justify-between gap-4">
          ...
          <input
            placeholder="Search roles (e.g., QA, Turvo, Zoho)"
        ...
        <div className="mt-6 grid md:grid-cols-2 gap-6">
          {filteredJobs.map((j) => (
            <div key={j.id} className="rounded-2xl border p-5 hover:shadow">
              ...
              <p className="mt-1 text-sm text-slate-600">{j.location} • {j.exp}</p>
              <p className="mt-3">{j.description}</p>
              <div className="mt-4 flex gap-3">
                <a href={`#apply-${j.id}`} className="rounded-xl bg-teal-600 px-4 py-2 text-white">Apply</a>
                <button
                  className="rounded-xl border px-4 py-2"
                  onClick={() => setActiveBlog("careers-bpo-us-logistics-mohali")}
                >
                  Learn More
                </button>
              </div>
              ...
              <form id={`apply-${j.id}`} className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <button type="submit" className="mt-2 rounded-xl bg-teal-600 px-4 py-2 text-white md:col-span-2">Submit Application</button>
              </form>
            </div>
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            ...
            <article key={e.title} className="rounded-2xl border p-5 hover:shadow">
              ...
              <a className="mt-3 inline-block text-teal-700 font-semibold" href="#">Read more →</a>
            </article>
          ))}
        </div>
      </Section>
      ...
      <Section id="blog" title="Blog & Stories" muted>
        {!blog ? (
          ...
          <article className="prose max-w-none">
            <button className="mb-4 text-teal-700" onClick={() => setActiveBlog(null)}>← Back to all posts</button>
            <h1>{blog.title}</h1>
      ...
      <Section id="contact" title="Contact & India Operations">
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div>
            ...
            <a href="#careers" className="mt-3 inline-block rounded-xl bg-teal-600 px-4 py-2 text-white">Partner / Apply</a>
          </div>
          <div>
...
function Markdown({ content }: { content: string }) {
  // ultra‑lite renderer for demo purposes
  return (
    <div className="prose">
      {content.split("\n\n").map((block, i) => {
        if (block.startsWith("# ")) {
